    ...
    if (user && firestore) {
      const checkUserRoleAndRedirect = async () => {
        ...
        try {
          ...
          // Expected for non-admins, continue
        }
        ...
        try {
          ...
          if (docSnap.exists()) {
            ...
            // User is authenticated but has no document, stay on login to be safe.
...
function LoginForm() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const docSnap = await getDoc(userDocRef);
      if (docSnap.exists()) {
        const role = (docSnap.data().role as UserRole) || 'student';
        router.push(`/dashboard/${role}`);
      } else {
         router.push('/login'); // Should not happen if user exists
      }
    } catch (error: any) {
